...
namespace ppsha.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        public HomeController(AppSettings settings, IOptions<MySettings> myConstants)
        {
            baseURL = settings.StorageConnectionString;
            MySettings = myConstants.Value;
            claimApiClient = new ClaimApiClient(baseURL);
            userApiClient = new UserApiClient(baseURL, MySettings);
        }
        public async Task<string> CreateClaim()
        {
            return "";
            //SessionObj.Init(HttpContext.Session);
            //bool isSignedin = await userApiClient.UserLogin();
            //if (isSignedin)
            //{
            ...
            //        var unProcessedClaimIds = await claimApiClient.GetUnprocessedClaimIds();
            //        var processedClaimIds = await claimApiClient.GetProcessedClaimIds();
            ...
            //                    claimData.UserId = SessionObj.UserId;
            //                    claimData.ClientJson = clientJsonStr;
            //                    var claimResult = await claimApiClient.CreateClaimApiClient(claimData);
            ...
            //                var processResult = await claimApiClient.ClaimOCRAsyncApiClientForMultiplePdf(claimRequestModel);
            //            }
            //        }
            //    }
            //    return "";
        ...
        //        {
        //            var structuredJson = await claimApiClient.GetStructuredJsonByClaimId(up.ClaimId.ToString());
        ...
        //    {
        //        var updateClaimResult = await claimApiClient.UpdatePostedClaimIds(processedUpdateClaimIds.ToArray());
        //    }
        //    if (unProcessedClaimIds.Count > 0 && unProcessedClaimIds != null)
        //    {
